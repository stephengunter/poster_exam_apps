<template>
   <v-data-table :headers="headers" :items="models" hide-default-footer item-key="index" :server-items-length="models ? models.length : 0">
      <template v-slot:item.subject="{ item }">
         <span v-if="item.subject" v-text="item.subject.title"></span>
      </template>
      <template v-slot:item.points="{ item }">
         <span v-if="item.points" v-text="item.points"></span>
      </template>
      <template v-slot:item.optionCount="{ item }">
         <span v-if="item.optionCount" v-text="item.optionCount"></span>
      </template>
