<template>
   <v-container fluid grid-list-xl fill-height>
       <v-layout justify-center  align-center>
			<v-flex xs12>
				<material-card>
					<notice-header ref="noticeHeader" :params="params" :can_create="canCreate"
					:open_options="openOptions" :active_options="activeOptions"
					@submit="onParamsChanged" @create="onCreate"
					/>
					<v-layout row wrap>
						<v-flex xs12>
							<notice-table :list="viewList"
							@off="onOff" @edit="onEdit"
							/>
						</v-flex>
					</v-layout>
					<v-flex xs12>
						<core-table-pager v-if="viewList.length > 0"
...
export default {
   ...
   computed: {
		...
		noticeHeader() {
			if(this.$refs.noticeHeader) return this.$refs.noticeHeader;
			else if (this.references.noticeHeader) return this.references.noticeHeader;
			return null;
		},
		viewList() {
			if(this.pagedList) {
